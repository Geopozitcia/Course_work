...
{
  char colors[][10] = {"0;30", /* Black */ "1;30",  /* Dark Gray */
  ...
  {
    printf("Поиск в подкатологах '%s%sm%s%s0m'  в директории %s%sm%s%s0m\n", CSI,
  ...
  else
  {
    ...
    {
      printf("Поиск '%s%sm%s%s0m' в директории %s%sm%s%s0m\n", CSI,
    ...
    else
    {
      ...
      {
        printf("Вы вызываете неверную операцию\n");
        return -2;
      }
    }
  }
  ...
  if (temp < 1)
  {
    printf("Вы ищите по пустому шаблону\n");
    return -5;
  }
  ...
  if (match_counter == 0)
  {
    ...
    printf("%s%sm%d%s0m совпадения в '%s%sm%s%s0m'\n", CSI, colors[3],
